...
const Iinterfaces_1 = require("../Iinterfaces");
...
class Table {
    ...
    createTable() {
        ...
        this.fields.forEach(curField => {
            ...
            // set type of field 
            if (Iinterfaces_1.stringType(curField.type)) {
            ...
            else if (Iinterfaces_1.numberType(curField.type)) {
                __fields = __fields + " int";
                if (curField.type.autoIncrement) {
                    __fields = __fields + "  AUTO_INCREMENT";
                }
            }
            else if (Iinterfaces_1.dateType(curField.type)) {
                __fields = __fields + " date";
            }
            else if (Iinterfaces_1.textType(curField.type)) {
                __fields = __fields + " text";
            }
            else if (Iinterfaces_1.booleanType(curField.type)) {
                __fields = __fields + " tinyint(1)";
            }
            else if (Iinterfaces_1.doubleType(curField.type)) {
                __fields = __fields + " double";
            }
            else if (Iinterfaces_1.enumType(curField.type)) {
            ...
            if (curField.default !== undefined) {
                if (Iinterfaces_1.CURRENT_TIMESTAMP(curField.default)) {
                    __fields = __fields + " DEFAULT CURRENT_TIMESTAMP";
                }
                else if (Iinterfaces_1.NULL(curField.default)) {
                    __fields = __fields + " DEFAULT NULL";
                }
                else if (Iinterfaces_1.AS_DEFINED(curField.default)) {
