...
class Table {
     ...
     */
    relatetable(thisTableFieldName, otherTable, otherTablefieldName, onDelete, onUpdate) {
        let __sql = `ALTER TABLE ${this.tableName} ADD KEY ${otherTable}_${this.tableName} (${thisTableFieldName}); 
        ALTER TABLE ${this.tableName} ADD CONSTRAINT ${otherTable}_${this.tableName} FOREIGN KEY (${thisTableFieldName}) REFERENCES ${otherTable} (${otherTablefieldName}) ON DELETE ${onDelete} ON UPDATE ${onUpdate}`;
        return __sql.trim();
    }
    validate() {
        return true;
